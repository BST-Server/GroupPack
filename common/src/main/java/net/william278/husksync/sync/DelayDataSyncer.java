...
public class DelayDataSyncer extends DataSyncer {
    ...
    public void syncApplyUserData(@NotNull OnlineUser user) {
        plugin.runAsyncDelayed(
                () -> {
                    // Priority 1: Check if user is switching servers (consume the switch marker)
                    if (!getRedis().getUserServerSwitch(user)) {
                        // Not switching servers: MySQL → Redis → Player
                    ...
                    // Is switching servers: listen for data from Redis (consume mode for cross-server transfer)
