...
public class LockstepDataSyncer extends DataSyncer {
    ...
    // Load from MySQL → write to Redis → apply to player
    @Override
    public void syncApplyUserData(@NotNull OnlineUser user) {
        this.listenForRedisData(user, () -> {
            ...
            // Core flow: MySQL → Redis → Player
            // 1. Load latest data from MySQL into Redis cache
            loadFromMysqlToRedis(user);
            // 2. Read from Redis (no-consume, as Redis is now the active cache) and apply to player
    ...
    public void syncSaveUserData(@NotNull OnlineUser onlineUser) {
        // Save to both Redis + MySQL on disconnect (persistent storage)
        plugin.runAsync(() -> saveUserDataOnDisconnect(
                onlineUser,
                DataSnapshot.SaveCause.DISCONNECT,
                (user, data) -> {
