...
public abstract class DataSyncer {
    ...
    /**
     * Save a user's current data to Redis only.
     * Used for real-time item changes and world saves (not persisted to MySQL).
     *
    ...
    /**
     * Save a user's current data to both Redis and MySQL (persistent).
     * Used for player disconnect and server shutdown scenarios.
     *
     * @param onlineUser the user to save data of
     * @param cause      the save cause (typically DISCONNECT or SERVER_SHUTDOWN)
     */
    public void saveUserDataOnDisconnect(@NotNull OnlineUser onlineUser, @NotNull DataSnapshot.SaveCause cause) {
        this.saveUserDataOnDisconnect(onlineUser, cause, (user, data) -> getRedis().setUserData(user, data));
    }

    /**
     * Save a user's current data to both Redis and MySQL (persistent) with a custom after-callback.
     *
     * @param onlineUser the user to save data of
     * @param cause      the save cause
     * @param after      consumer to run after data is saved to MySQL + Redis
     */
    public void saveUserDataOnDisconnect(@NotNull OnlineUser onlineUser, @NotNull DataSnapshot.SaveCause cause,
                                         @Nullable BiConsumer<User, DataSnapshot.Packed> after) {
        this.saveData(onlineUser, onlineUser.createSnapshot(cause), after);
    }
    ...
    // Save data to Redis only (for real-time sync: item changes, world saves)
    @Blocking
    private void saveToRedis(@NotNull User user, @NotNull DataSnapshot.Packed data) {
        getRedis().setUserData(user, data);
        plugin.debug("[%s] Data saved to Redis (real-time sync)".formatted(user.getName()));
    }

    // Save data to both Redis and MySQL (for persistent storage: disconnect, shutdown)
    @Blocking
    private void addSnapshotToDatabase(@NotNull User user, @NotNull DataSnapshot.Packed data,
                                       @Nullable BiConsumer<User, DataSnapshot.Packed> after) {
        // 1. Sync to Redis (for cross-server handoff and real-time access)
        getRedis().setUserData(user, data);
        // 2. Persist to MySQL (required as persistent storage)
        getDatabase().addSnapshot(user, data);
        plugin.debug("[%s] Data persisted to MySQL + Redis".formatted(user.getName()));
    ...
    /**
     * Load a user's latest data from MySQL into Redis cache.
     * Called when a player joins so that Redis has the authoritative data from MySQL.
     *
     * @param user the online user to load data for
     */
    @Blocking
    protected void loadFromMysqlToRedis(@NotNull OnlineUser user) {
        try {
            ...
            plugin.log(Level.WARNING, "Failed to load %s's data from MySQL into Redis"
