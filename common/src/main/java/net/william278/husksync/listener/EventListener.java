...
public abstract class EventListener {
    ...
    public void handlePluginDisable() {
        // Save for all online players (persist to MySQL + Redis on shutdown)
        plugin.getOnlineUsers().stream()
                .filter(user -> !plugin.isLocked(user.getUuid()) && !user.isNpc())
                .forEach(user -> {
                    plugin.lockPlayer(user.getUuid());
                    plugin.getDataSyncer().saveUserDataOnDisconnect(
                            user, DataSnapshot.SaveCause.SERVER_SHUTDOWN);
                });
