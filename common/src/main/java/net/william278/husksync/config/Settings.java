...
public class Settings {
    ...
    public static class SynchronizationSettings {
        ...
        @Comment({"Real-time inventory synchronization settings.",
                "When enabled, all inventory changes (click, drag, pickup, drop) are immediately synced to Redis.",
                "Redis becomes the single source of truth for player inventory data."})
        private InventorySyncSettings inventorySync = new InventorySyncSettings();

        @Comment("MySQL database auto-backup settings")
        ...
        public static class InventorySyncSettings {
            ...
            @Comment("Enable full Redis inventory takeover mode (game does not manage inventory data)")
            private boolean enabled = true;

            @Comment("Debounce interval in ticks between syncs (default: 10 ticks = 500ms). "
                    + "Increase for large servers to reduce CPU load.")
            private long debounceTicks = 10;

            @Comment("Periodic enforcement interval in seconds (default: 5). "
                    + "Forces all online players' inventories to match Redis. "
                    + "Set to 0 to disable periodic enforcement.")
        ...
        public static class BackupSettings {
            ...
            @Comment("Whether to enable automatic MySQL database backups")
            private boolean enabled = true;

            @Comment("Backup interval in minutes (default: 360 minutes = 6 hours)")
            private long intervalMinutes = 360;

            @Comment("Directory name for storing backup files (relative to plugin data folder)")
