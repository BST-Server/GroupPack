...
public class RedisManager extends JedisPubSub {
    ...
    /**
     * Fetch a user's data from Redis without consuming (deleting) the key.
     * Used when Redis is the primary data store rather than a temporary cache.
     *
     * @param user The user to fetch data for
     * @return The user's data, if it's present on Redis. Otherwise, an empty optional.
     */
    @Blocking
    public Optional<DataSnapshot.Packed> getUserDataNoConsume(@NotNull User user) {
        try (Jedis jedis = jedisPool.getResource()) {
            final byte[] key = getKey(RedisKeyType.LATEST_SNAPSHOT, user.getUuid(), clusterId);
            final byte[] dataByteArray = jedis.get(key);
            if (dataByteArray == null) {
                plugin.debug(String.format("[%s] No %s key found on Redis (no-consume)",
                        user.getName(), RedisKeyType.LATEST_SNAPSHOT));
                return Optional.empty();
            }
            plugin.debug(String.format("[%s] Read %s key from Redis (no-consume)",
                    user.getName(), RedisKeyType.LATEST_SNAPSHOT));

            // Do NOT consume (delete) the key - Redis is now the primary store
            return Optional.of(DataSnapshot.deserialize(plugin, dataByteArray));
        } catch (Throwable e) {
            plugin.log(Level.SEVERE, "An exception occurred getting a user's data from Redis (no-consume)", e);
            return Optional.empty();
        }
    }
